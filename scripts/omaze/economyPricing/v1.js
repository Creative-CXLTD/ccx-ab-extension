(function () {
  ...
  const handleNoSourcePaygCards = (CONTROL_NO_SOURCE_PAYG_CARDS) => {
    ...
    CONTROL_NO_SOURCE_PAYG_CARDS.elements.forEach(originalButton => {
      try {
        ...
        if (entryCountEl) entryCountEl.textContent = `${newCount} Entries`;
  ...
  // ------------------------------------
  // NEW: ROUTER FOR THE CORRECT SELECTOR
  // ------------------------------------
  ...
  const init = () => {
    try {
      ...
      if (sourceType === "FACEBOOK") {
        selectorToUse = SELECTORS.CONTROL_FACEBOOK_SOURCE_PAYG_CARDS;
      } else if (sourceType === "OTHER") {
        selectorToUse = SELECTORS.CONTROL_OTHER_SOURCE_SOURCE_PAYG_CARDS;
      } else {
        selectorToUse = SELECTORS.CONTROL_NO_SOURCE_PAYG_CARDS;
      }

      waitForElements(
        [{ selector: selectorToUse, count: expectedCount }],
        function (results) {
          ...
          customLog(resultGroup);

          handleNoSourcePaygCards(resultGroup);
        }
      );
