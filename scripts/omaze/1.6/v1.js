(function () {
  ...
  const TEST_ID = "1_6";
  const TEST_NAME = "Circle Back Video on House LP";
  const VARIATION = "variation-1";
  const CURRENT_URL = window.location.href;
  
  const SELECTORS = {
    CONTROL_CAMPAIGN_MULTI_BLOCK_ELEMENT: '[id*="__campaign-multi-block"]',
  }
  ...
  const init = () => {
    try {
      ...
      waitForElements(
        [
          { selector: SELECTORS.CONTROL_CAMPAIGN_MULTI_BLOCK_ELEMENT, count: 1 },
        ],
        function (results) {
          ...
          const CONTROL_CAMPAIGN_MULTI_BLOCK_ELEMENT = results[0].elements[0];
          if (!CONTROL_CAMPAIGN_MULTI_BLOCK_ELEMENT) return;
          customLog(CONTROL_CAMPAIGN_MULTI_BLOCK_ELEMENT);
        }
      );
