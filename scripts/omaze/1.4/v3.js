(function () {
  ...
  const SELECTORS = {
    ...
    CONTROL_MOBILE_HERO_LAST_H3: '.hero-mobile-addendum > .hma-content > h3:last-of-type',
    CONTROL_DESKTOP_HERO_CTA: '.home--banner .campaign-hero__content .w-100',
  }
...
/* Each slide is absolutely positioned for fade transitions */
.ccx-slider__slide {
  position: absolute;
  top: 0;
  left: 0;
...
.ccx-slider__slide--active {
  opacity: 1;
  z-index: 1;
  pointer-events: auto;
}

/* Keeps your desktop layout intact */
.ccx-slider__content {
  display: flex;
  width: 100%;
  gap: 1rem;
...
.shopify-section .ccx-slider__image {
...
.ccx-slider__arrow--left {
  left: 12px;
}

.ccx-slider__arrow--right {
  right: 12px;
}

/* ----------------------------- */
/* SLIDER - DESKTOP LAYOUT */
/* ----------------------------- */
@media (min-width: 768px) {
  ...
  /* make wrapper transparent on desktop */
  .ccx-slider__wrapper {
    background: transparent !important;
  }

  /* and apply translucent background only to each slide */
  .ccx-slider__slide {
    background: #081F28A6 !important;
  }
}
...
.ccx-sticky-banner {
  ...
  const SVG_TRUSTPILOT_STAR_AND_LABEL = `<svg width="85" height="26" viewBox="0 0 105 26" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`;
  ...
  function createSlider(options) {
    ...
    var version = options.version || "desktop";
    var showTrustpilot = options.showTrustpilot || false;
    var showImage = options.showImage || false;
    var TEST_ID = options.TEST_ID;

    var anchorEl = document.querySelector(container);
    if (!anchorEl) return console.error("Slider container missing:", container);

    // ----- Create actual slider root -----
    var sliderRoot = document.createElement("div");
    sliderRoot.classList.add("ccx-slider", "ccx-" + TEST_ID);

    // Insert AFTER the anchor element
    anchorEl.insertAdjacentElement("afterend", sliderRoot);

    // ----- Create wrapper -----
    var wrapper = document.createElement("div");
    wrapper.className = "ccx-slider__wrapper";

    // ----- Build slides -----
    slides.forEach(function (slide) {
      var slideEl = document.createElement("div");
      slideEl.className = "ccx-slider__slide ccx-slider__slide--" + version;
      ...
      if (showTrustpilot) {
        trustpilotHTML =
          '<div class="ccx-slider__trustpilot">' +
          SVG_TRUSTPILOT_STAR_AND_LABEL +
          SVG_TRUSTPILOT_STARS +
          '</div>';
      }

      slideEl.innerHTML =
        '<div class="ccx-slider__content">' +

        '<div class="ccx-slider__left">' +
        leftContent +
        '</div>' +

        '<div class="ccx-slider__right">' +
        '<p class="ccx-slider__main-text">“' + slide.mainText + '”</p>' +

        '<p class="ccx-slider__user">' +
        '<strong>' + slide.name + '</strong>, ' +
        '<span class="ccx-slider__user-text">' + slide.userInfo + '</span>' +
        '</p>' +
    ...
    // Collect slide elements for fade logic
    var slideElements = Array.prototype.slice.call(wrapper.children);

    // ----- Fade Logic -----
    var currentIndex = 0;

    slideElements[0].classList.add("ccx-slider__slide--active");

    function updateWrapperHeight() {
      var activeSlide = slideElements[currentIndex];
    ...
    function showSlide(index) {
      slideElements.forEach(function (slide, i) {
        slide.classList.toggle("ccx-slider__slide--active", i === index);
      });
    ...
    arrowLeft.className = "ccx-slider__arrow ccx-slider__arrow--left";
    arrowLeft.innerHTML = ARROW_LEFT;

    var arrowRight = document.createElement("button");
    arrowRight.className = "ccx-slider__arrow ccx-slider__arrow--right";
    arrowRight.innerHTML = ARROW_RIGHT;

    sliderRoot.appendChild(arrowLeft);
    sliderRoot.appendChild(arrowRight);
  ...
  const init = () => {
    try {
      ...
      // Wait for CONTROL_MAIN_NAV to load and attach the sticky banner
      waitForElements(
        [
      ...
      // Wait for CONTROL_MOBILE_HERO_LAST_H3 to load and create the mobile slider
      waitForElements(
        [
          { selector: SELECTORS.CONTROL_MOBILE_HERO_LAST_H3, count: 1 },
        ],
        function (results) {
          ...
          const CONTROL_MOBILE_HERO_LAST_H3 = results[0].elements[0];
          if (!CONTROL_MOBILE_HERO_LAST_H3) return;
          customLog(CONTROL_MOBILE_HERO_LAST_H3);

          createSlider({
            container: SELECTORS.CONTROL_MOBILE_HERO_LAST_H3,
            slides: SLIDE_DATA,
            version: "mobile",
      ...
      // Wait for CONTROL_DESKTOP_HERO_CTA to load and create the desktop slider
      waitForElements(
        [
        ...
        function (results) {
          ...
          if (!CONTROL_DESKTOP_HERO_CTA) return;
          customLog(CONTROL_DESKTOP_HERO_CTA);

          createSlider({
            container: SELECTORS.CONTROL_DESKTOP_HERO_CTA,
            slides: SLIDE_DATA,
            version: "desktop",
            showTrustpilot: false,
            TEST_ID,
