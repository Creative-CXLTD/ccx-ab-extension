...
`;
  ...
  const createGuaranteedWinnerAndOddsComponent = (CONTROL_HOME_BANNER_CONTROLS, className) => {
    ...
    component.innerHTML =
      ...
      ICON_TROPHY +
      '</div>' +
      '<div class="ccx-guaranteed-winner__text">' +
      '<span>1 garantierter Gewinnerin</span>' +
      '</div>' +
      '</div>' +
      '<div class="ccx-odds">' +
      '<div class="ccx-odds__icon">' +
      ICON_INFORMATION +
      '</div>' +
    ...
    CONTROL_HOME_BANNER_CONTROLS.insertAdjacentElement('afterend', component);

    customLog('[createGuaranteedWinnerAndOddsComponent] Component successfully inserted.');
  };
  ...
  const init = () => {
    try {
      ...
      waitForElements(
        [
          { selector: SELECTORS.CONTROL_HOME_BANNER_CONTROLS, count: 1 },
        ...
        (results) => {
          ...
          // Element References
          // ==========================
          const CONTROL_HOME_BANNER_CONTROLS = results?.[0]?.elements?.[0];
          const CONTROL_HMA_LAST_H3 = results?.[1]?.elements?.[0];
          if (!CONTROL_HOME_BANNER_CONTROLS || !CONTROL_HMA_LAST_H3) return;

          customLog('[Elements Ready]', { CONTROL_HOME_BANNER_CONTROLS, CONTROL_HMA_LAST_H3 });
          ...
          document.addEventListener('click', (e) => {
            const trigger = e.target.closest('.ccx-odds');
            if (trigger) {
              e.preventDefault();
