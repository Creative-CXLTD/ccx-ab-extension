...
@media screen and (min-width: 1255px) {
  ...
  // V1/V2: “< 24 Std.” (or “< 1 TAG”)
  // V3: “Noch < 1 Tag” --> FOR VARIATION 3, CHANGE TO Noch < 24 Std.'
  // const endDate = new Date(Date.now() + 12 * 60 * 60 * 1000);

  // already expired
  // Always returns < 24 Std.
  // V3 shows Noch < 1 Tag
  ...
  const getRemainingDaysNumber = () => {
    // Variation 3 uses its own helper
    if (VARIATION === 'variation-3') {
      const { number } = getV3NumberAndUnit();
      return number;
    }

    // Original behaviour for other variations (if ever needed)
  ...
  const getRemainingTimeText = () => {
    ...
    // For V1 + V2 wording: 1 Tag / N Tagen
    if (diffDays === 1) return "1 Tag";
    return diffDays + " Tagen";
  };

  // Dedicated helper for Variation 3: returns number + unit ("5 Tage", "1 Tag", "< 1 Tag")
  const getV3NumberAndUnit = () => {
    const now = new Date();

    const diffMs = endDate.getTime() - now.getTime();
    const diffHours = diffMs / (1000 * 60 * 60);

    // Expired or < 24h -> show "< 24 Std."
    if (diffMs <= 0 || diffHours < 24) {
      return { number: "< 24", unit: "Std." };   // Noch < 24 Std.
    }

    const diffDays = Math.floor(diffHours / 24);

    if (diffDays === 1) {
      return { number: "1", unit: "Tag" };     // Noch 1 Tag
    }

    return { number: String(diffDays), unit: "Tage" }; // Noch N Tage
  };
  ...
  const updateCountdownText = () => {
    ...
    if (VARIATION === 'variation-2') {
      const span = document.querySelector(
        '.ccx-de16-variation-2 .ccx-top-row span:nth-child(2)'
      );
      if (span) span.textContent = remaining.toUpperCase();
    }

    if (VARIATION === 'variation-3') {
      const { number, unit } = getV3NumberAndUnit();

      const numberSpan = document.querySelector(
        '.ccx-de16-variation-3 .ccx-bottom-left span:nth-child(1)'
      );
      const unitSpan = document.querySelector(
        '.ccx-de16-variation-3 .ccx-bottom-left span:nth-child(2)'
      );
  ...
  const createDesktopCountdownContainer = (CONTROL_NAVBAR_TOGGLE_MENU) => {
    ...
    if (VARIATION === 'variation-1') {
      ...
      bottomP.innerHTML =
        'Bonus-Verlosung <span>Endet in</span> <span>' +
    ...
    if (VARIATION === 'variation-2') {
      ...
      topP.innerHTML =
        'Bonus-Verlosung <span>ENDET IN</span> <span>' +
        getRemainingTimeText().toUpperCase() +
        '</span>';
      topRow.appendChild(topP);
    ...
    if (VARIATION === 'variation-3') {
      ...
      leftP.innerHTML = 'Noch <span>' + number + '</span> <span>' + unit + '</span>';
  ...
  const createMobileCountdownContainer = (CONTROL_MOBILE_CTA) => {
    ...
    if (VARIATION === "variation-3") {
      ...
      mobileBox.innerHTML =
        ...
        'Noch <span>' + number + '</span> <span>' + unit + '</span>' +
        '</button>';
    }
